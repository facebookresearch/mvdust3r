...
def get_3D_model_from_scene(outdir, silent, scene, min_conf_thr=3, as_pointcloud=False, mask_sky=False,
    ...
    rgbimg = scene.imgs # rgbimg[0] (384, 512, 3) 0~1
    focals = scene.get_focals().cpu() # (N,)
    cams2world = scene.get_im_poses().cpu() #  (N, 4, 4)
    # 3D pointcloud from depthmap, poses and intrinsics
    pts3d = to_numpy(scene.get_pts3d()) # [0] (384, 512, 3)
    scene.min_conf_thr = float(scene.conf_trf(torch.tensor(min_conf_thr)))
    msk = to_numpy(scene.get_masks()) # [0] (384, 512)
...
def get_reconstructed_scene(outdir, model, device, silent, image_size, filelist, schedule, niter, min_conf_thr,
    ...
    output = inference(pairs, model, device, batch_size=batch_size, verbose=not silent) # dict_keys(['view1', 'view2', 'pred1', 'pred2', 'loss'])
    ...
    confs = to_numpy([c for c in scene.im_conf]) # [0] (384, 512, 4) 0~1
